...
contract ImpulseQuickSwap3pool is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function depositInUnderlying(uint256[] calldata _amounts)
        external
        nonReentrant
    ...
    function _rebalanceAmounts(uint256[] memory _amounts) internal returns (uint256[] memory rebalanceAmounts) {
        ...
        (uint256[] memory excessAmounts, uint256[] memory missingAmounts) = _getExcessAndMissingAmounts(
            usdToken,
            _amounts,
            index
        );
    ...
    function _calculateRebalanceDivider(address _usdToken, uint256 _usdIndex)
        internal
        view
        returns (uint256 rebalanceDivider)
    {
        for (uint256 i = 0; i < underlyings.length; i++) {
            if (_usdIndex == i) {
                rebalanceDivider += 1 ether * _getTokenPoolMultiplier(i);
            } else {
                rebalanceDivider += _getTokenPriceInUsd(
                    i,
                    _getTokenDecimals(i) * _getTokenPoolMultiplier(i),
                    _usdToken
                );
            }
        }
    ...
    function _getExcessAndMissingAmounts(
        ...
        uint256 tokensRebalanceAmount = (underlyingsTotal * (1 ether)) /
            _calculateRebalanceDivider(_usdToken, usdIndex);
    ...
    function _swapAllWantToOneUnderlying(address _underlying, address _receiver) internal {
        ...
        for (uint256 i = 0; i < underlyings.length; i++) {
            ...
            _swapTokens(
                swapUnderlyingRoutes[address(underlyings[i])][_underlying],
                underlyingsBalances[i],
                slippageTolerance
            );
        }
    ...
    function _swapUSDTokenToUnderlyings(uint256[] memory _missingAmounts, address usdToken) internal {
        ...
        for (uint256 u = 0; u < underlyings.length; u++) {
            ...
            _swapTokensForExact(
                swapUnderlyingRoutes[usdToken][address(underlyings[u])],
                _missingAmounts[u],
                slippageTolerance
            );
        }
