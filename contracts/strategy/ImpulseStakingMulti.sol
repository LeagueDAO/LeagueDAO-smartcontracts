...
// File: contracts\interfaces\IImpulseMultiStrategy.sol
...
// File: contracts\interfaces\IImpulseMultiStrategyQuick.sol
...
// File: contracts\staking\ImpulseStakingMulti.sol
...
contract ImpulseStakingMulti is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function updatePool(uint256 _pid) public hasPool(_pid) {
        ...
        for (uint256 i = 0; i < pool.rewardsTokens.length; i++) {
            ...
            pool.accumulatedPerShare[i] =
                pool.accumulatedPerShare[i] +
                (unaccountedReward * pool.accuracy[i]) /
                pool.poolSupply;
        }
        pool.lastRewardBlock = block.number;
    }
    ...
    function depositInUnderlying(uint256 _pid, uint256[] calldata _amounts)
        public
        virtual
        nonReentrant
        hasPool(_pid)
    ...
    function _transferCommission(uint256 _pid, address _underlying)
        internal
        returns (uint256 underlyingBalance, uint256 withdrawCommissions)
    {
    ...
    function userPoolAmountInUsd(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (_pid == 3) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                userTotalAmounts[i] = ((user.shares *
                    IImpulseMultiStrategyQuick(address(strategy)).wantLockedTotalForEach()[i]) /
