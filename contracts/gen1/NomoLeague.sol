...
contract NomoLeague is OwnableUpgradeable {
    ...
    /// @dev Token in form of which rewards are payed, v1 version, don't use now
     ...
     * @dev Current game is ID of the game when player has interacted with contract for the last time (used for
     * calculations)
     */
    struct Player {
    ...
    /// @dev Reward per one active point, magnified by 2**128 for precision
    uint256 private _magnifiedRewardPerPoint;

    /// @dev Mapping of game IDs to values of magnifiedRewardPerPoint at their end (used for calculations)
    mapping(uint256 => uint256) private _rewardPerPointAfterGame;

    /**
     * @dev Mapping of addresses to their corrections of the rewards (used to maintain rewards unchanged when number of
     * player's points change)
     */
    mapping(address => int256) private _magnifiedRewardCorrections;

    /// @dev Mapping of addresses to their withdrawn reward amounts
    ...
    /// @dev Reward token position => game IDs => values of magnifiedRewardPerPoint at their end (used for calculations)
    mapping(uint256 => mapping(uint256 => uint256)) internal _v2_rewardPerPointAfterGame;

    /**
     * @dev Reward token position => user address => corrections of the rewards (used to maintain rewards unchanged when
     * number of player's points change)
     */
    mapping(uint256 => mapping(address => int256)) internal _v2_magnifiedRewardCorrections;

    //// @dev Reward token position => user address => withdrawn reward amounts
    mapping(uint256 => mapping(address => uint256)) internal _v2_rewardWithdrawals;

    //// @dev Helpful flag for provide update to new version
    uint256 internal _version;

    // EVENTS

    /// @dev Event emitted when user withdraws his reward
    event RewardsWithdrawn(address indexed account, uint256[] amounts);

    /// @dev Event emitted when new game starts
    event NewGameStarted(uint256 indexed index);

    /// @dev Event emitted when token is staked to the league
    event TokenStaked(address indexed account, uint256 indexed tokenId);

    /// @dev Event emitted when token is unstaked from league
    event TokenUnstaked(address indexed account, uint256 indexed tokenId);

    /// @dev Event emitted when user's active points change
    event ActivePointsChanged(uint256 newPoints);

    /// @dev Event emitted when token points update
    event UpdatePoints(
     ...
     * @param router_ NomoRouter contract
     * @param name_ Name of the league
     * @param totalGames_ Total number of games in the league
     * @param tokenLimitPerPlayer_ Maximal number of tokens one player can stake
     */
    function initialize(
        INomoRouter router_,
        string memory name_,
     ...
     * @notice account Account address
     */
     ...
     * @return Reward per one active point, magnified by 2**128 for precision
     */
     ...
     * @return Value of magnifiedRewardPerPoint at their end
     */
     ...
     * @notice Correction of the reward
     */
     ...
     * @param rewardIndex Reward index
     * @param account Account address
     * @notice Withdrawn reward amount
     */
     ...
     * @param _newLimit New limit value
     */
    ...
    /**
     * @dev This function updates reward per point, distributing reward and then converts pending points from current
     * game to active for next game
     * @param totalRewards Rewards to distribute for current game
     */
    ...
    /**
     * @dev Calculate points
     * @notice tokenId ID of the token
     */
    ...
    /**
     * @dev Restrict access only to the router address
     */
    modifier onlyRouter() {
