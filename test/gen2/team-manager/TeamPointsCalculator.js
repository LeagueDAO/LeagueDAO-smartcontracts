...
contract("TeamPointsCalculator", function () {
    ...
    describe("[Staker]", () => {
        ...
        it.skip("Cannot unstake player with zero possition", async () => {
            await gen2PlayerToken.connect(users[0]).approve(teamManager.address, TOKEN_ID);
            await teamManager.connect(users[0]).stakePlayer(TOKEN_ID);

            await teamManager.connect(users[0]).unstakePosition(TOKEN_ID);
        });
